...
export const getPersons = createAsyncThunk('persons/fetch', async (value: {}) => {
    const {getData} = useAxios();
    try {
	const api = await getData("http://localhost:8080/getPerson");
...
interface CounterState {
    persons: [{name: string,updatedAt: string,surname: string,money: 0,info: string,mobNumber: string,payment: [{status: string,money: number,sumOfMoney: number,date: {year: number,month: number,day: number,hour: number,minute: number},info: string}],status: string,_id: string}] | [],
    userStatus: boolean,
  }
