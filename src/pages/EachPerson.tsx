    ...
    <div>
      <div className='flex justify-between items-end'>
       ...
       {state.length > 0 && <p onClick={() => navigate(`/updatePersonInfo/${state[0]._id}`)} className='mr-[10px] text-gray-400 underline cursor-pointer'>განახლება</p>}
        ...
        <div className='mt-[50px] ml-[3px]'>
           <div className='text-[20px]'>{state[0].name} {state[0].surname}</div>
           <div className='text-[20px] mt-[10px]'>ვალი - {state[0].money}ლ </div>
           ...
           <div className='max-w-[270px] mx-auto mt-[30px] border-[1px] p-[5px] bg-[#95a5a6] rounded-[10px]'>
                <EachPersonForm id={state[0]._id} />
              </div> 
              <div className='mt-[20px]'>
      {state[0].payment.map((d,index) => {
        return (
       <div  key={index} className={` ${d.status === "add" ? "bg-[#2ecc71]" : d.status === "edit" ? "bg-[#3498db]" : "bg-[#e74c3c]"} text-white rounded-[10px] text-center  max-w-[270px] mx-auto mt-[30px] min-h-[50px]  cursor-pointer flex justify-between items-center px-[4px] text-[16px] flex-wrap`}>
        ...
        {d.info && <div className='text-start'><p>დამატებითი ინფორმაცია - {d.info}</p></div>}
      </div>
        )
      })
}
