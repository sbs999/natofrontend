...
const Sulakauri = () => {
    ...
    const [currentBooks,setCurrentBooks] = useState(sulakauriBooks);

    useEffect(() => {
      setCurrentBooks(sulakauriBooks);
    },[sulakauriBooks])
  
    useEffect(() => {
      if(search) {
     const filteredInfo = sulakauriBooks.filter(a =>
  ...
  return (
    ...
    <div className='flex justify-center flex-col items-center mt-[15px]'>
        <div  className='text-[20px]'>გამომცემლობა სულაკაური</div>
        <div onClick={() => navigate("/books/sulakauri/addbooks")} className='mt-[10px] text-gray-500 cursor-pointer text-[18px]'>წიგნების დამატება</div>
      ...
      <div className='mt-[10px]'>
        ...
        return (
       <div onClick={() => navigate(`/books/sulakauri/${d.code}`)} key={index} className='bg-[#00b894] text-white text-bold rounded-[10px] p-[1%] text-center  w-[280px]  text-[17px] mt-[5px] flex flex-col  cursor-pointer'>
       ...
       return (
      <div  key={index} className='bg-[#00b894] text-white text-bold rounded-[10px] p-[1%] text-center  w-[280px]  text-[17px] mt-[5px] flex flex-col  cursor-pointer'>
      <div className='text-center'> <p>{d.name} </p></div>
       <div className='flex mt-[10px] justify-between px-[1%]'>
